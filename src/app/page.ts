...
export default function IntroPage(): JSX.Element {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setButtonTextIndex((prev: number) => (prev + 1) % buttonTexts.length);
    }, 700);
  ...
  const getButtonBackgroundColor = (): string => {
    return buttonTextIndex === 1 ? "#D4D4D4" : "#F56156";
  };
    ...
    >
      ...
      >
        <div className="relative">
          ...
          {/* 런치챗 보내기 버튼 */}
          <motion.div
            className="absolute left-1/2 transform -translate-x-1/2 bottom-74 md:bottom-82"
            style={buttonStyle}
          >
